...
chao1<-chao1$data %>% rownames_to_column("sample-id")
simpson <- read_tsv("simpson/alpha-diversity.tsv")
simpson<-simpson$data %>% rownames_to_column("sample-id")
...
pielou<-pielou$data %>% rownames_to_column("sample-id")
ace <- read_tsv("ace/alpha-diversity.tsv")
ace<-ace$data %>% rownames_to_column("sample-id")
```
3-Merge table
```{r}
